...
class BaseEmbedder(ABC):
    ...
    def get_query_embedding(self,state):
...
class DimTruncator(BaseEmbedder):
    ...
    def truncate_faiss_exact(self,*args, **kwargs):
        ...
        self.logger.info(
            "Loaded original FAISS index: ntotal=%d, dim=%d", nb, d_old
        )
        ...
        self.logger.info(
            "Completed FAISS truncation: ntotal=%d, dim_old=%d, dim_new=%d, time=%.2fs",
            nb, d_old, d_new, total_time
        )
